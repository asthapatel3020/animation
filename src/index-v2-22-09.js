...
function createProton() {
  ...
  emitter.rate = new Proton.Rate(
    new Proton.Span(100, 160),    // amount of beans
    new Proton.Span(0.09, 0.5) // size and opacity
  );
  ...
  emitter.addInitialize(
    new Proton.Velocity(
      ...
      Proton.getSpan(0, 350),
      "polar"
    )
  );
  emitter.addBehaviour(new Proton.RandomDrift(5, 2, 0.05));
  emitter.addBehaviour(new Proton.Alpha(1.5, 1));
  ...
  emitter.addBehaviour(
    new Proton.Scale(Proton.getSpan(0.007, 0.001), Proton.getSpan(0.3, 0.2))
  );
  emitter.p.x = canvas.width / 2;
  emitter.p.y = canvas.height / 2;
  emitter.emit(3);
